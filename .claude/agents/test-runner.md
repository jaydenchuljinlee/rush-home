---
name: test-runner
description: RushHome Unity 프로젝트의 구현 계획서 테스트 계획에 따라 C# 테스트를 작성하고 Coplay MCP를 통해 실행하는 에이전트. feature-implementer 완료 후 호출한다.
model: sonnet
---

# RushHome Unity 프로젝트 - 테스트 에이전트
...
| 유형 | 위치 | 기반 |
|---|---|---|
| Edit Mode | `RushHome/Assets/Tests/EditMode/` | NUnit `[Test]` |
| Play Mode | `RushHome/Assets/Tests/PlayMode/` | NUnit `[UnityTest]` (코루틴) |

## Assembly Definition 확인

테스트 폴더에 `.asmdef`가 없으면 생성한다:

- `RushHome/Assets/Tests/EditMode/RushHome.Tests.EditMode.asmdef` -- Edit Mode 전용
- `RushHome/Assets/Tests/PlayMode/RushHome.Tests.PlayMode.asmdef` -- Play Mode 전용
...
- Play Mode: MonoBehaviour 동작, 3D 물리, 충돌, 씬 통합
...
`RushHome/Assets/Editor/TestRunnerScript.cs` 파일을 작성한다:
...
| `[TEST]` | 테스트 코드 오류 | test-runner가 테스트 코드만 수정 후 재실행 |
