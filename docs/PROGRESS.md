...
| F-06 | 난이도 곡선 | F-04 | 완료 | 30초/60초/120초마다 체감 난이도 상승 |
| F-13 | 레벨 패턴 다양화 | F-06 | 완료 | 장애물 조합 패턴 + 난이도별 패턴 풀 + AirObstacle 좌우 이동 + 거리 기반 스폰 |
| F-14 | 지형 변화 (경사/틈새) | F-13 | 완료 | GroundScroller 타일 순환에 경사/틈새 타일 통합, 장애물 Y가 지면 높이 추종 |
...
| F-09 | 메인메뉴 씬 | F-05 | 완료 | 메인메뉴에서 게임 씬으로 전환 |

### 디자인

| # | Feature | 선행 | 상태 | 완료 기준 |
|---|---------|------|------|----------|
| F-07 | 배경 파랄랙스 | F-02 | 완료 | 3겹 배경이 서로 다른 속도로 스크롤 |
| F-08 | 캐릭터 애니메이션 | F-03 | 완료 | 상태별 애니메이션 재생 |
